...
def hash(request, h):
    ...
    if j.get('extra') and j['extra']['status'] == 'deleted':
        raise Http404('I am sorry :( The hash is deleted at %s.' %
                      j['extra']['update_time'])
    d = {'info': j}
    d['keywords'] = list(
        set(re_punctuations.sub(u' ', d['info']['name']).split()))
    if 'files' in d['info']:
        d['info']['files'] = [y for y in d['info'][
            'files'] if not y['path'].startswith(u'_')]
        d['info']['files'].sort(key=lambda x: x['length'], reverse=True)
    d['related'] = Hash.objects.list_related(
        d['info']['id'], d['info']['name'])
...
def search(request, keyword=None, p=None):
    ...
    d['offset'] = d['ps'] * (d['p'] - 1)
    try:
        res = Hash.objects.search(keyword, d['offset'], d[
                                  'ps'], d['category'], d['sort'])
    except:
    ...
    if ids:
        ...
        for x in d['result']['items']:
            for y in items:
                if x['id'] == y['id']:
                    x.update(y)
                    x['maybe_fake'] = x['name'].endswith(u'.rar') or u'BTtiantang.com' in x[
                        'name'] or u'liangzijie' in x['name'] or u'720p高清视频' in x['name']
                    if 'files' in x:
                        x['files'] = [z for z in x['files'] if not z['path'].startswith(u'_')][
                            :5]
                        x['files'].sort(
